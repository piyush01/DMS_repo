...
public class EditCommunitiesServlet extends DSpaceServlet
{
            ...
            SQLException, AuthorizeException
    {
    	...
    	if(action!=0 && action==START_CREATE_COMMUNITY){
            ...
            SQLException, AuthorizeException
    {
        ...
        case START_EDIT_COMMUNITY:
          ...
          /*  Community[] subcommunities = community.getSubcommunities();*/
            if(request.getParameter("modify").equals("modify"))
        	{           		
        		request.setAttribute("default_modify","modify");
        	}
        ...
        case START_CREATE_COMMUNITY:
        	...
        	Community topcommunity1[]=Community.findAllTop(context);
           
             request.setAttribute("topcommunity", topcommunity1);
            JSPManager.showJSP(request, response, "/tools/edit-community.jsp");
        ...
        case CONFIRM_DELETE_COMMUNITY:
            ...
            if (parent == null)
            {               
             response.sendRedirect(response.encodeRedirectURL(request.getContextPath()+"?message="+msg));
            }
            else
            // redirect to parent community page
            {
                response.sendRedirect(response.encodeRedirectURL(request
                        .getContextPath()
                        + "/handle/" + parent.getHandle()+"?message="+msg));
            }
        ...
        case CONFIRM_DELETE_COLLECTION:
            ...
            // Show main control page
            showDeleteControls(context, request, response);
            ...
            SQLException, AuthorizeException
    {
        ...
        else if (community != null)
        {
        	 ...
        	 if (request.getParameter("create").equals("true"))
             {
        		 ...
        		 int parentCommID = UIUtil.getIntParameter(request,
                         "parent_community_id");
                 if (parentCommID == -1)
                 {
                   response.sendRedirect(response.encodeRedirectURL(request.getContextPath() + "/dspace-admin/create-folder?action=cabinet&message="+msg));
                 }
                 else
                 {
        		 response.sendRedirect(response.encodeRedirectURL(request
                         .getContextPath()
                         + "/handle/" + community.getHandle())+"?message="+msg); 
                 }
        	 }
        	 else if (request.getParameter("create").equals("false"))
        	 {
        		 msg="update";
        		 response.sendRedirect(response.encodeRedirectURL(request
                         .getContextPath()
        ...
        else
        {
            ...
            if (parent != null)
            {
            	msg="Cabinet has been successfully created!";
            	request.setAttribute("message", "Cabinet has been successfully created!"); 
                response.sendRedirect(response.encodeRedirectURL(request
                        .getContextPath()
                        + "/handle/" + parent.getHandle()+"?message="+msg));
    ...
    private void showDeleteControls(Context context, HttpServletRequest request,
            HttpServletResponse response) throws ServletException, IOException,
            SQLException, AuthorizeException
    {            	
        ...
        if (request.getParameter("create").equals("true"))
        {
            ...
            if (parentCommunityID != -1)
            {
                ...
                if (parent != null)
                {
                    ...
                    {
        				eperson=EPerson.find(context,Integer.parseInt(userid));
                    }
                    if(group!=null && Integer.parseInt(group)!=0)
                    {
            				 groups=Group.find(context,Integer.parseInt(group));
...
/*        HarvestedCollection hc = HarvestedCollection.find(context, collection.getID());
      ...
      /*  if (button.equals("submit_set_logo"))
        ...
        if (button.startsWith("submit_wf_create_"))
        {
