...
public class FileTag extends TagSupport
{
    ...
    /** log4j logger */
    private static Logger log = Logger.getLogger(FileTag.class);
    ...
    public int doStartTag() throws JspException
    {
        try
        {
            ...
            {
                style = styleSelection.getStyleForItem(item1);
            }
     ...
     *            the item to display
     */
    public void setItem1(Item itemIn)
    {
    	item1 = itemIn;
    }
    ...
    private void listBitstreams() throws IOException
    {
        ...
        try
        {
        	Bundle[] bundles = item1.getBundles("ORIGINAL");
        	...
        	{
        		boolean html = false;
        		String handle = item1.getHandle();
        		Integer item_id = item1.getID();
        		Bitstream primaryBitstream = null;
        		Bundle[] bunds = item1.getBundles("ORIGINAL");
        		Bundle[] thumbs = item1.getBundles("THUMBNAIL");
                log.info("item_id:-----------"+item_id);
        		// if item contains multiple bitstreams, display bitstream
        		// description
        		boolean multiFile = false;
        		Bundle[] allBundles = item1.getBundles();
        		out.print("<input type=\"hidden\" value=\""+item_id+"\" name=\"item_id\" id=\"item_id\"/>");
        		...
        		for (int i = 0; i < bundles.length; i++)
        		{
        			...
        			log.info("total file :------------------"+bitstreams.length);
        			...
        			{        
        				if (!bitstreams[k].getFormat().isInternal())
        				{
                         ...
                         out.print("<input type=\"hidden\" value=\""+bitstreams[k].getID()+"\" name=\"bitstream_id\" id=\"bitstream_id\"/>");
                         out.print("<input type=\"hidden\" value=\""+bitstreams[k].getSize()+"\" name=\"file_size\" id=\"fid\"/>");
       					 out.print("<input type=\"hidden\" value=\""+bitstreams[k].getName()+"\" name=\"file_name\" id=\"fid\"/>");
       					 out.print("<input type=\"hidden\" value=\""+bsLink+"\" name=\"file_link\" id=\"fid\"/>");
       					 out.print("<input type=\"hidden\" value=\""+bitstreams[k].getSequenceID()+"\" name=\"file_no\" id=\"fid\"/>");               
        				}
        			}
        		}
            	...
            	if (html)
            	{
            		...
            		/*out.print("<td class=\"standard\"><a class=\"btn btn-primary\" target=\"_blank\" href=\"");
            		out.print(request.getContextPath());
            		out.print("/html/");
            		out.print(handle + "/");
            		out
            		...
            		for (int i = 0; i < bundles.length; i++)
            		{
            			...
            			for (int k = 0; k < bitstreams.length; k++)
            			{
            				...
            				if (!bitstreams[k].getFormat().isInternal())
            				{
            					...
            					out.print("</td>");            					
            					out.print("<td class=\"standard\" align=\"center\">");
    ...
    private void showLicence() throws IOException
    {
        ...
        try
        {
        	bundles = item1.getBundles("LICENSE");
        }
        catch(SQLException sqle)
        {
        	throw new IOException(sqle.getMessage(), sqle);
